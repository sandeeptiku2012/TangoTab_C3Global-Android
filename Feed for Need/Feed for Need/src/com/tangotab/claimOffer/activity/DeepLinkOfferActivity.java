 ...
 * 
 */
public class DeepLinkOfferActivity extends FacebookActivity implements
OnClickListener, Handler.Callback {
	...
	private static final int EMPTY_DEEPLINK_ID = 999;
	...
	private DealsDetailVo mDealsDetailVo;
	...
	public void onCreate(Bundle savedInstances){
		...
		//						Intent intent =  new Intent(DeepLinkOfferActivity.this, FacebookLogin.class);
		...
		//				}
		//				else{
		//					Toast.makeText(DeepLinkOfferActivity.this, "Invalid Deep link", Toast.LENGTH_SHORT).show();
		//				}
		//			}
		//		}
		//
		...
		if (!ValidationUtil.isNull(mDealsDetailVo)) {
			setData(mDealsDetailVo);
		}
		...
		claimNow.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if (checkInternetConnection())
				{
					...
					SharedPreferences user = getSharedPreferences("UserName", 0);
					String userName = user.getString("username", "");
					mDealsDetailVo.setUserId(userName);
					...
					if (!ValidationUtil.isNullOrEmpty(spMailingId)&& !ValidationUtil.isNullOrEmpty(spUserId)
						...
						offerImage.setImageUrl(mDealsDetailVo.getImageUrl(),VolleySingleton.getInstance(DeepLinkOfferActivity.this).getImageLoader());
					}
				}
				//				} 
				//				else {
		...
		map.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if (checkInternetConnection()) {
					...
					mapIntent.putExtra("selectDeal", mDealsDetailVo);
					mapIntent.putExtra("businessname",mDealsDetailVo.getBusinessName());
					mapIntent.putExtra("dealname", mDealsDetailVo.getDealName());
					mapIntent.putExtra("address", mDealsDetailVo.getAddress());
		...
		addToCalender.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				calIntent.putExtra("title","TangoTab at " + mDealsDetailVo.getBusinessName());
				calIntent.putExtra("description",mDealsDetailVo.getDealDescription());
				if (!ValidationUtil.isNullOrEmpty(mDealsDetailVo.getAddress())) {
					try {
						String[] splitAdd = mDealsDetailVo.getAddress().split(",");
						if (splitAdd != null) {
							...
							calIntent.putExtra("eventLocation",	mDealsDetailVo.getAddress());
						}
		...
		getDirection.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (checkInternetConnection()) {
					...
					mapIntent.putExtra("selectDeal",(DealsDetailVo) mDealsDetailVo);
					mapIntent.putExtra("fromPage", "claimOffer");
					mapIntent.putExtra("address", mDealsDetailVo.getAddress());
					mapIntent.putExtra("businessname",mDealsDetailVo.getBusinessName());
					mapIntent.putExtra("dealname", mDealsDetailVo.getDealName());
		...
		topMenuBtn.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent meIntent = new Intent(DeepLinkOfferActivity.this,MainMenuActivity.class);
		...
		meButton.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent meIntent = new Intent(DeepLinkOfferActivity.this,MeActivity.class);
				meIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
				startActivity(meIntent);
			}
		});
		...
		nearMe.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent nearmeIntent = new Intent(DeepLinkOfferActivity.this,NearMeActivity.class);
		...
		topSearchBtn.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent searchIntent = new Intent(DeepLinkOfferActivity.this,SearchActivity.class);				
	...
	public class DealDetailsAsyncTask extends AsyncTask<LoginVo, Void, List<DealsDetailVo>> {
		...
		public DealDetailsAsyncTask(DeepLinkInputsVo deepLinkInputsVo) {
			dialog = new ProgressDialog(DeepLinkOfferActivity.this);
		...
		protected void onPostExecute(List<DealsDetailVo> dealsList) {
			...
			if(dealsList != null && dealsList.size() > 0){
				...
				mDealsDetailVo = dealsList.get(0);
			}
			else if(dealsList != null && dealsList.size() == 0){
				showDialog(EMPTY_DEEPLINK_ID);
			}
		}
	}
	...
	public class InsertDealAsyncTask extends AsyncTask<DealsDetailVo, Void, Void> {
		...
		protected void onPreExecute() {
			if (!isFinishing()) {
				mDialog = ProgressDialog.show(DeepLinkOfferActivity.this,"Claiming ", "Please wait...");
	...
	AsyncTask<LoginVo, Void, List<OffersDetailsVo>> {
		...
		protected void onPostExecute(List<OffersDetailsVo> offersList) {
			...
			application = (TangoTab) getApplication();
			AutoCheckinActivity activity = new AutoCheckinActivity(DeepLinkOfferActivity.this,getApplicationContext(), offersList);
			activity.doCheckIn();
		}
	}
	...
	protected Dialog onCreateDialog(int id) {
		switch (id) {
		case Message_Id:
			AlertDialog.Builder ab = new AlertDialog.Builder(DeepLinkOfferActivity.this);
		...
		case EMPTY_DEEPLINK_ID:
			AlertDialog.Builder ab_emptydeep = new AlertDialog.Builder(DeepLinkOfferActivity.this);
			ab_emptydeep.setTitle("TangoTab");
			ab_emptydeep.setMessage("Sorry, this deal is expired");
			ab_emptydeep.setPositiveButton("OK", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					dialog.dismiss();
					finish();
				}
			});
			return ab_emptydeep.create();

		case Reser_comp:
			AlertDialog.Builder ab1 = new AlertDialog.Builder(DeepLinkOfferActivity.this);
			...
			return ab1.create();
		case 10:
			AlertDialog.Builder ab2 = new AlertDialog.Builder(DeepLinkOfferActivity.this);
		...
		case AppConstant.FACEBOOK_POST_SUCCESSFUL_DIALOG:
			AlertDialog.Builder facebookShareDialog = new AlertDialog.Builder(DeepLinkOfferActivity.this);
		...
		case TWITTER_ID:
			AlertDialog.Builder twitterDialog = new AlertDialog.Builder(DeepLinkOfferActivity.this);
		...
		case TWEET_SENT:
			AlertDialog.Builder tweetSent = new AlertDialog.Builder(DeepLinkOfferActivity.this);
	...
	public class SendTweetAsyncTask extends AsyncTask<Void, Void, Integer> {
		...
		protected Integer doInBackground(Void... params) {
			try {
				...
				TwitterUtils.sendTweet(prefs, mDealsDetailVo.getBusinessName()+" offer on TangoTab" + "\n"
						+ mDealsDetailVo.getDealName() + "\n"
						+" \n http://www.tangotab.com/jsp/dealSummary.do?dealId="+ mDealsDetailVo.getId()
						+"&date="+ mDealsDetailVo.getDealEndDate());
			...
			try {
				...
				TwitterUtils.sendTweet(prefs,
						strBuilder.toString() + " is planning to claim "
								+ mDealsDetailVo.getBusinessName() + " "
								+ mDealsDetailVo.getDealDescription() + " "
								+ mDealsDetailVo.getDealName());
				return SUCCESS;
			} catch (Exception e) {
				if (isClaimClicked) {
	...
	class OAuthRequestTokenTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			try {
				...
				// twitterHandler.sendEmptyMessage(TWITTER_FAILED);
				if (!isFinishing()) {
					DeepLinkOfferActivity.this.runOnUiThread(new Runnable() {
			...
			public boolean shouldOverrideUrlLoading(WebView view, String url) {
				...
				if (url.startsWith("htt")) {
					...
					SharedPreferences prefs = PreferenceManager
							.getDefaultSharedPreferences(DeepLinkOfferActivity.this);
					...
					if (uri != null
						...
						new RetrieveAccessTokenTask(DeepLinkOfferActivity.this,
	...
	public boolean onKeyDown(int keycode, KeyEvent e) {
		...
		case KeyEvent.KEYCODE_MENU:
			Intent mainMenuIntent = new Intent(DeepLinkOfferActivity.this,
					MainMenuActivity.class);
			mainMenuIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
			startActivity(mainMenuIntent);
			return true;
		case KeyEvent.KEYCODE_SEARCH:
			Intent searchIntent = new Intent(DeepLinkOfferActivity.this,
	...
	private void publishMessageCalim() {
		...
		facebookFacade.publishMessage(strBuilder.toString()
				+ " is planning to claim  " + mDealsDetailVo.getBusinessName()
				+ "\n " + mDealsDetailVo.getDealDescription(),
				"images.tangotab.com", mDealsDetailVo.getBusinessName(), null,
				mDealsDetailVo.getImageUrl(), null);
	...
	private void publishMessage() {
		...
		facebookFacade.publishMessage(mDealsDetailVo.getBusinessName()+" offers on TangoTab.",
				"http://www.tangotab.com/jsp/dealSummary.do?dealId="+mDealsDetailVo.getId()+
				"&date="+mDealsDetailVo.getDealEndDate(),
				mDealsDetailVo.getDealDescription(),
				mDealsDetailVo.getAddress(), mDealsDetailVo.getImageUrl());
	}
	...
	Handler shareHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case AppConstant.CLAIM_OFFER:
				new InsertDealAsyncTask().execute(mDealsDetailVo);
	...
	public void postoTwitterClaim() {
		...
		// Start twitter activity if user is not logged in
			 ...
			 */if (!isFinishing()) {
				 if (progressDialog == null) {
					 progressDialog = new ProgressDialog(DeepLinkOfferActivity.this);
					 progressDialog.setMessage("Please wait..");
					 progressDialog.setCancelable(false);
				 }
				 progressDialog.show();
	...
	protected void onResume()
	{
		...
		if(data != null){
			...
			if(scheme.equalsIgnoreCase("ttapp") && host.equalsIgnoreCase("deal")){
				if( params.size() > 0){
					Toast.makeText(getApplicationContext(), params.get(0), Toast.LENGTH_SHORT).show();
					deepLinkInputsVo.setDeal_ID(params.get(0)); //815
					deepLinkInputsVo.setLatitude(locationLatitude); 
					deepLinkInputsVo.setLongitude(locationLongitude);
					mDeepLinkInputsVo = deepLinkInputsVo;
