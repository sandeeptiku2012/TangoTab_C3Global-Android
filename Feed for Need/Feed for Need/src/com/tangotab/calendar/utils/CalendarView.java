...
import com.tangotab.calendar.utils.CalendarWrapper.OnDateChangedListener;
...
public class CalendarView extends LinearLayout 
{
	...
	public static int prevMonth,prevYear,prevDay;
	CalendarDayMarker currentDay;
	...
	public static void setPrevDay(int prevDay) {
	prevDay = prevDay;
	}
		...
		for(int i = 0; i < dayItemsInGrid; i++) {
			...
			for (int j = 0; j < markers.length; j++) {
				...
				if (tempCal.get(Calendar.YEAR) == m.getYear() && tempCal.get(Calendar.MONTH) == m.getMonth() && day == m.getDay()) {
					...
					tv.setTextColor(m.getColor());
					tv.setBackgroundResource(m.getResource());
					hits++;
					break;
				}
			}
	...
	private void init(Context context) {
		...
		setView(MONTH_VIEW);
		setDay(_calendar.getDay());
		setMonth(_calendar.getMonth());
		setYear(_calendar.getYear());
		currentDay=new CalendarDayMarker(_calendar.getYear(), _calendar.getMonth(), _calendar.getDay(), Color.WHITE,R.drawable.currentdayimg);
		Log.e("",""+_calendar.getMonth()+","+prevYear);
		if(prevYear!=0)
		{
			CalendarDayMarker prevDayMarker=new CalendarDayMarker(prevYear,prevMonth,prevDay, Color.BLACK, R.drawable.date_blue);	
			setDaysWithEvents(new CalendarDayMarker[]{currentDay,prevDayMarker});
		}
		else{
			setDaysWithEvents(new CalendarDayMarker[]{currentDay});
		}
		//setDaysWithEvents(new CalendarDayMarker[] { new CalendarDayMarker(_calendar.getYear(), _calendar.getMonth(), _calendar.getDay(), Color.BLACK, R.drawable.date_blue) });
	...
	private OnClickListener _incrementClicked = new OnClickListener() {
		public void onClick(View v) {
			...
			/*CalendarDayMarker currentDay=new CalendarDayMarker(_calendar.getYear(), _calendar.getMonth(), _calendar.getDay(), Color.BLACK, R.drawable.date_blue);
			if(prevYear!=0){
				CalendarDayMarker prevDayMarker=new CalendarDayMarker(prevYear,prevMonth,prevDay, Color.BLACK, R.drawable.date_blue);	
				setDaysWithEvents(new CalendarDayMarker[]{currentDay,prevDayMarker});
			}
	...
	private OnClickListener _dayClicked = new OnClickListener() {
		public void onClick(View v) {
			...
			prevDay=Integer.parseInt(day);
			prevMonth=_calendar.getMonth();
			prevYear=_calendar.getYear();
			...
			setYear(_calendar.getYear());
			refreshDayCells();
			Log.e("prev",""+prevYear+","+prevMonth+","+prevDay);
			if(prevYear!=0){
				CalendarDayMarker prevDayMarker=new CalendarDayMarker(prevYear,prevMonth,prevDay, Color.BLACK, R.drawable.date_blue);	
				setDaysWithEvents(new CalendarDayMarker[]{currentDay,prevDayMarker});
			}
			...
			//setDaysWithEvents(new CalendarDayMarker[] { new CalendarDayMarker(year1, Integer.parseInt(month)-1, day1, Color.WHITE, R.drawable.date_blue) });
	...
	private void refreshDayCells() {
		...
		Log.e("day grid length",""+dayGrid.length);
		int monthAdd = -1;
		int row = 1; // Skip weekday header row
		int col = 0;
		...
		for (int i = 0; i < dayGrid.length; i++) {
			...
			tv.setBackgroundResource(R.drawable.date_gray);
			
			
			tv.setText(dayGrid[i] + "");
			
			if (monthAdd == 0)
				tv.setTextColor(Color.parseColor("#004ee7"));
			
			
			else{
					tv.setTextColor(Color.LTGRAY);
					tv.setBackgroundResource(R.drawable.date_white);
				}
