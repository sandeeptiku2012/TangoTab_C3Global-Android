...
public class SearchActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (!ValidationUtil.isNullOrEmpty(restSearch
				.getString("restSearch", "")))
		...
		if (bundel != null) {
			...
			if (!ValidationUtil.isNullOrEmpty(fromPage)
				...
				if (!ValidationUtil.isNullOrEmpty(address)) {
					...
					DealsDetailVo dealsDetailVo = (DealsDetailVo) bundel
							.getSerializable("selectDeal");
		...
		nearMe.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (AppConstant.IS_CLAIM_OFFER_PAGE) {
					...
					Intent calimedIntent = new Intent(SearchActivity.this,
							ClaimOfferActivity.class);
	...
	private String getTextFromDate(int year, int month, int day) {
		...
		if (dateSelect.equalsIgnoreCase(new SimpleDateFormat("MMM d yyyy ")
				.format(current)))
			result = "Today";
		else if (dateSelect
				.equalsIgnoreCase(new SimpleDateFormat("MMM d yyyy ")
						.format(tomorrowDate)))
			result = "Tomorrow";
		else {
			result = dateSelect;
