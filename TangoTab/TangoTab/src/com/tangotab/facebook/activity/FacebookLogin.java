...
public class FacebookLogin extends FacebookActivity {
	...
	AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			if (user != null) {
				try {
					...
					edits11.putBoolean("facebookStatus", response.get("facebook_share").equalsIgnoreCase("true") ? true : false);
					edits11.putBoolean("twitterStatus", response.get("twitter_share").equalsIgnoreCase("true") ? true : false);
					edits11.commit();
				} 
				catch (Exception e) {
		...
		protected void onPostExecute(Void result) {
			...
			else if (!ValidationUtil.isNull(message) && message.equalsIgnoreCase("")
	...
	public void getIntoActivity(String uriPath, String parameter) {
			...
			if (index != -1) {
				...
				try {
					...
					Log.v("spMailingId ,spUserdId ,spJobId from dealSilver pop link",
							"spMailingId =" + param[2] + " spUserdId=" + param[3] + " spJobId=" + param[4]);
				} 
		...
		if (uriPath.equalsIgnoreCase("deal")) {
			...
			if (!ValidationUtil.isNullOrEmpty(parameter)) {
				...
				dealId = param[0];
				date = param[1];
				Log.v("Deal id and Deal deal date from URL is ", 
						"dealId =" + dealId + 
						" Deal Date " + date);
				/**
				 * Set the spMailingid,spUserId and spJobId into session.
		...
		protected Void doInBackground(Void... params) {
			try {
				...
				try {
					...
					message = null;
					Log.e("ConnectTimeoutException occured", "ConnectTimeoutException occured at the time of facebook signup process", e2);
					Intent contactIntent = new Intent(getApplicationContext(), ContactSupportActvity.class);
					startActivity(contactIntent);
				} 
				catch (Exception e1) {
	...
	public StringEntity signupDetailsRequest() {
		...
		try {
			...
			//System.out.println("-- Inputs to Server --" + writer.toString());
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == PICK_ZIPCODE) {
			if (resultCode == RESULT_OK) {
				...
				workZip = data.getStringExtra(AppConstant.ZIP_CODE_WORK);
