...
public class ZipActivity extends Activity 
{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final EditText promoCodeEditext = (EditText) findViewById(R.id.zipactivity_edittext_promocode);
		...
		createNewAccount.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v){
				...
				String promoCode = promoCodeEditext.getText().toString();
				...
				if(!ValidationUtil.isNullOrEmpty(homeZipCode) && !ValidationUtil.isNullOrEmpty(workZipCode)){
					if(!homeZipMatcher.matches()){
						...
						resultIntent.putExtra(AppConstant.ZIP_CODE_WORK, workZipCode);
						resultIntent.putExtra(AppConstant.PROMO_CODE, promoCode);
				...
				else if(ValidationUtil.isNullOrEmpty(promoCode)){
					showDialog(5);
				}
			}
		});
	...
	protected Dialog onCreateDialog(int id) {
		switch (id) {
		case 1:
			AlertDialog.Builder homeZipErrDialog = new AlertDialog.Builder(	ZipActivity.this);
			homeZipErrDialog.setTitle("TangoTab");
			homeZipErrDialog.setMessage("Invalid Home Zip code.");
			homeZipErrDialog.setPositiveButton("OK", new DialogInterface.OnClickListener() {
		...
		case 2:
			AlertDialog.Builder workZipErrDialog = new AlertDialog.Builder(ZipActivity.this);
			workZipErrDialog.setTitle("TangoTab");
			workZipErrDialog.setMessage("Invalid Work Zip code.");
			workZipErrDialog.setPositiveButton("OK", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					dialog.dismiss();
				}
			});
		...
		case 3:
			AlertDialog.Builder homeZipEmptyDialog = new AlertDialog.Builder(ZipActivity.this);
			homeZipEmptyDialog.setTitle("TangoTab");
			homeZipEmptyDialog.setMessage("Please Specify Home Zip code.");
			homeZipEmptyDialog.setPositiveButton("OK", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					dialog.dismiss();
				}
			});
		...
		case 4:
			...
			workZipEmptyDialog.setPositiveButton("OK", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					dialog.dismiss();
				}
			});
		...
		case 5:
			AlertDialog.Builder promoCodeEmptyDialog = new AlertDialog.Builder(ZipActivity.this);
			promoCodeEmptyDialog.setTitle("TangoTab");
			promoCodeEmptyDialog.setMessage("Please Specify Promo Code.");
			promoCodeEmptyDialog.setPositiveButton("OK", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					dialog.dismiss();
				}
			});
			return promoCodeEmptyDialog.create();
		}
		return null;
	}
}